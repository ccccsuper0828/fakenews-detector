...
class Run():
    def __init__(self,
        ...
        if config['dataset']=="weibo":
            # Use main.py --root_path (e.g. Colab: /content/drive/MyDrive/data 3/)
            rp = config.get('root_path') or './data/'
            rp = os.path.abspath(os.path.expanduser(str(rp).strip()))
            self.root_path = rp if rp.endswith(os.sep) else rp + os.sep
