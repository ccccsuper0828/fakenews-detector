    ...
    for c, res in res_by_category.items():
        try:
            metrics_by_category[c] = {
                'auc': roc_auc_score(res['y_true'], res['y_pred']), 4) if hasattr(roc_auc_score, '__call__') else 0
            }
        ...
        try:
            metrics_by_category[c] = {
                'precision': precision_score(res['y_true'], np.around(np.array(res['y_pred'])).astype(int),
                                             average='macro'), 4) if hasattr(roc_auc_score, '__call__') else 0,
...
def metrics(y_true, y_pred, category, category_dict):
    ...
    for c, res in res_by_category.items():
        try:
            metrics_by_category[c] = {
                'auc': _safe_round(roc_auc_score(res['y_true'], res['y_pred']))
            }
        except ValueError:
            metrics_by_category[c] = {'auc': 0}

    try:
        metrics_by_category['auc'] = _safe_round(roc_auc_score(y_true, y_pred, average='macro'))
    except ValueError:
        metrics_by_category['auc'] = 0

    y_pred_bin = np.around(np.array(y_pred)).astype(int)
    metrics_by_category['metric'] = _safe_round(f1_score(y_true, y_pred_bin, average='macro'))
    metrics_by_category['recall'] = _safe_round(recall_score(y_true, y_pred_bin, average='macro'))
    metrics_by_category['precision'] = _safe_round(precision_score(y_true, y_pred_bin, average='macro'))
    metrics_by_category['acc'] = _safe_round(accuracy_score(y_true, y_pred_bin))

    for c, res in res_by_category.items():
        res_pred = np.around(np.array(res['y_pred'])).astype(int)
        metrics_by_category[c] = {
            'precision': _safe_round(precision_score(res['y_true'], res_pred, average='macro', zero_division=0)),
            'recall': _safe_round(recall_score(res['y_true'], res_pred, average='macro', zero_division=0)),
            'fscore': _safe_round(f1_score(res['y_true'], res_pred, average='macro', zero_division=0)),
            'auc': metrics_by_category.get(c, {}).get('auc', 0),
            'acc': _safe_round(accuracy_score(res['y_true'], res_pred)),
        }
    return metrics_by_category
class Recorder():
